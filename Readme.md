...
# Метод Every
## Метод every()instances Arrayпроверяет, все ли элементы массива проходят тест, реализованный предоставленной функцией. Он возвращает логическое значение.
![Image](./massive%20every.png)
# Метод Some
## Метод some()instances Arrayпроверяет, проходит ли хотя бы один элемент в массиве тест, реализованный предоставленной функцией. Он возвращает true, если в массиве он находит элемент, для которого предоставленная функция возвращает true; в противном случае он возвращает false. Он не изменяет массив.
![Image](./massive%20some.png)
# Метод With
## Метод with()экземпляров Array— это копирующая версия использования скобочной нотации для изменения значения заданного индекса. Он возвращает новый массив, в котором элемент по заданному индексу заменен заданным значением.
![Image](./massive%20with.png)
# Метод fill
## Метод fill()экземпляров Arrayизменяет все элементы в диапазоне индексов в массиве на статическое значение. Он возвращает измененный массив.
![Image](./massive%20fill.png)